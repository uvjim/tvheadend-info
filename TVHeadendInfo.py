...
class Mail:
    recpt = 'james@simpsoncomputers.co.uk'
    sender = 'james@simpsoncomputers.co.uk'
    pwd = 'rfushbonicguklfc'
    ...
    def __init__(self, body):
        ...
        self.mail['From'] = Mail.sender
        self.mail['To'] = Mail.recpt
        self.server = smtplib.SMTP(Mail.server, Mail.port)

    def send(self):
        self.server.starttls()
        self.server.login(Mail.sender, Mail.pwd)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, 'cd:f', ['clean', 'days=', 'dvrfailed'])
